...
def parse_option():
    parser = argparse.ArgumentParser(
        "command line arguments for fine-tuning schema item classifier."
    )
    ...
    parser.add_argument(
        "--device", type=str, default="0", help="the id of used GPU device."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train_filepath",
        type=str,
        default="data/pre-processing/preprocessed_train_spider.json",
        help="path of pre-processed training dataset.",
    )
    parser.add_argument(
        "--dev_filepath",
        type=str,
        default="data/pre-processing/preprocessed_dev.json",
...
def prepare_batch_inputs_and_labels(batch, tokenizer):
    ...
    (
        batch_input_tokens,
        batch_column_info_ids,
        batch_table_name_ids,
    ...
    for batch_id in range(batch_size):
        ...
        batch_column_number_in_each_table.append(
            [
                len(column_infos_in_one_table)
                for column_infos_in_one_table in column_infos_in_one_db
            ]
        )
    ...
    for batch_id in range(batch_size):
        ...
        if sum(batch_column_number_in_each_table[batch_id]) > len(
            ...
            truncated_column_number = sum(
                batch_column_number_in_each_table[batch_id]
            ) - len(batch_aligned_column_labels[batch_id])
            batch_column_number_in_each_table[batch_id][-1] -= truncated_column_number

    encoder_input_ids = tokenized_inputs["input_ids"]
    encoder_input_attention_mask = tokenized_inputs["attention_mask"]
    batch_aligned_column_labels = [
        torch.LongTensor(column_labels) for column_labels in batch_aligned_column_labels
    ]
    batch_aligned_table_labels = [
        torch.LongTensor(table_labels) for table_labels in batch_aligned_table_labels
    ]
    ...
    if torch.cuda.is_available():
        ...
        batch_aligned_column_labels = [
            column_labels.cuda() for column_labels in batch_aligned_column_labels
        ]
        batch_aligned_table_labels = [
...
def _train(opt):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        opt.model_name_or_path, add_prefix_space=True
    )

    train_dataset = SchemaItemClassifierDataset(opt.train_filepath)

    train_dataloder = DataLoader(
        train_dataset, batch_size=opt.batch_size, shuffle=True, collate_fn=lambda x: x
    )

    dev_dataset = SchemaItemClassifierDataset(opt.dev_filepath)

    dev_dataloder = DataLoader(
        dev_dataset, batch_size=opt.batch_size, shuffle=False, collate_fn=lambda x: x
    )

    # initialize model
    model = SchemaItemClassifier(
        model_name_or_path=opt.model_name_or_path, mode=opt.mode
    )
    ...
    optimizer = optim.AdamW(params=model.parameters(), lr=opt.learning_rate)
    ...
    for epoch in range(opt.epochs):
        ...
        for batch in train_dataloder:
            ...
            (
                ...
                batch_aligned_table_name_ids,
                batch_column_number_in_each_table,
            ) = prepare_batch_inputs_and_labels(batch, tokenizer)
            ...
            if train_step % num_checkpoint_steps == 0:
                ...
                for batch in tqdm(dev_dataloder):
                    ...
                    for batch_id, table_logits in enumerate(
                        model_outputs["batch_table_name_cls_logits"]
                    ):
                        table_pred_probs = torch.nn.functional.softmax(
                            table_logits, dim=1
                        )

                        table_pred_probs_for_auc.extend(
                            table_pred_probs[:, 1].cpu().tolist()
                        )
                        table_labels_for_auc.extend(
                            batch_table_labels[batch_id].cpu().tolist()
                        )

                    for batch_id, column_logits in enumerate(
                        model_outputs["batch_column_info_cls_logits"]
                    ):
                        column_pred_probs = torch.nn.functional.softmax(
                            column_logits, dim=1
                        )

                        column_pred_probs_for_auc.extend(
                            column_pred_probs[:, 1].cpu().tolist()
                        )
                        column_labels_for_auc.extend(
                            batch_column_labels[batch_id].cpu().tolist()
                        )

                # calculate AUC score for table classification
                table_auc = roc_auc_score(
                    table_labels_for_auc, table_pred_probs_for_auc
                )
                # calculate AUC score for column classification
                column_auc = roc_auc_score(
                    column_labels_for_auc, column_pred_probs_for_auc
                )
                ...
                if writer is not None:
                    writer.add_scalar(
                        "table AUC", table_auc, train_step / num_checkpoint_steps
                    )
                    writer.add_scalar(
                        "column AUC", column_auc, train_step / num_checkpoint_steps
                    )
                ...
                if toral_auc_score >= best_score:
                    ...
                    torch.save(
                        model.state_dict(), opt.save_path + "/dense_classifier.pt"
                    )
                    model.plm_encoder.config.save_pretrained(
                        save_directory=opt.save_path
                    )
...
def _test(opt):
    ...
    dataloder = DataLoader(
        dataset, batch_size=opt.batch_size, shuffle=False, collate_fn=lambda x: x
    )

    # initialize model
    model = SchemaItemClassifier(model_name_or_path=opt.save_path, mode=opt.mode)

    # load fine-tuned params
    model.load_state_dict(
        torch.load(
            opt.save_path + "/dense_classifier.pt", map_location=torch.device("cpu")
        )
    )
    if torch.cuda.is_available():
        model = model.cuda()
