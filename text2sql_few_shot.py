...
def prepare_input_ids_and_attention_mask(
    tokenizer, input_seq, max_input_length, device
):
...
def prepare_cross_domain_input_seq(opt, eval_data, demonstration_set, similarity):
    top_k_indices = sorted(
        range(len(similarity)), key=lambda x: similarity[x], reverse=True
    ...
    for idx in top_k_indices:
        ...
        input_seq += (
            demonstration_set[idx]["schema_sequence"]
            + "\n"
...
def text2sql_func(
    model, text2sql_input_seq, tokenizer, max_tokens, max_new_tokens, eos_token_id
):
    inputs = prepare_input_ids_and_attention_mask(
        tokenizer, text2sql_input_seq, max_tokens - max_new_tokens, model.device
    )
...
if __name__ == "__main__":
    ...
    eval_set_question_skeletons = [
        extract_skeleton(question) for question in eval_set_questions
    ]
    ...
    demonstration_set_question_skeletons = [
        extract_skeleton(question) for question in demonstration_set_questions
    ]

    print("length of demonstration set:", len(demonstration_set))

    demonstration_set = filter_schema(
        demonstration_set, "train", None, opt.table_num, opt.column_num
    )
    ...
    for demonstration_sample in demonstration_set:
        demonstration_sample["schema_sequence"] = get_db_schema_sequence(
            demonstration_sample["schema"]
        )
        demonstration_sample["content_sequence"] = get_matched_content_sequence(
            demonstration_sample["matched_contents"]
        )
    for eval_sample in eval_set:
        eval_sample["schema_sequence"] = get_db_schema_sequence(eval_sample["schema"])
        eval_sample["content_sequence"] = get_matched_content_sequence(
            eval_sample["matched_contents"]
        )
    ...
    question_similarities = simsce_model.similarity(
        eval_set_questions, demonstration_set_questions
    )
    question_skeleton_similarities = simsce_model.similarity(
        eval_set_question_skeletons, demonstration_set_question_skeletons
    )
    ...
    print(
        "tokenizer.decode(new_eos_token_id): '{}'".format(
    ...
    for eval_data_idx, eval_data in tqdm(enumerate(eval_set)):
        input_seq = prepare_cross_domain_input_seq(
            opt, eval_data, demonstration_set, similarities[eval_data_idx]
        )

        if eval_data_idx < 2:
            print(input_seq)

        generated_sqls = text2sql_func(
            model, input_seq, tokenizer, max_tokens, max_new_tokens, new_eos_token_id
        )
        generated_sqls = [
            post_process(generated_sql, eval_data["schema"]["schema_items"])
        ...
        for generated_sql in generated_sqls:
            execution_error = check_sql_executability(
                generated_sql, eval_data["db_path"]
            )
            if (
