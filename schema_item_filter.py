...
def prepare_inputs_and_labels(sample, tokenizer):
    ...
    column_num_in_each_table = [
        len(table["column_names"]) for table in sample["schema"]["schema_items"]
    ]
    ...
    for column_name_word_index in column_name_word_indices:
        column_name_token_indices.append(
            [
                token_id
                for token_id, word_index in enumerate(word_indices)
                if column_name_word_index == word_index
            ]
        )

    # obtain token indices of each table in `input_ids`
    for table_name_word_index in table_name_word_indices:
        table_name_token_indices.append(
            [
                token_id
                for token_id, word_index in enumerate(word_indices)
                if table_name_word_index == word_index
            ]
        )
        ...
        for column_name in column_names[table_idx]:
            if (
                ...
                splitted_samples.append(
                    {"text": text, "schema": get_schema(recorded_tables_and_columns)}
                )
                recorded_tables_and_columns = [[table_name, column_name]]
...
def filter_schema(dataset, dataset_type, sic, num_top_k_tables=5, num_top_k_columns=5):
    for data in tqdm(dataset, desc="filtering schema items for the dataset"):
        ...
        table_comments = [
            table["table_comment"] for table in data["schema"]["schema_items"]
        ]
        column_names = [
            table["column_names"] for table in data["schema"]["schema_items"]
        ]
        column_types = [
            table["column_types"] for table in data["schema"]["schema_items"]
        ]
        column_comments = [
            table["column_comments"] for table in data["schema"]["schema_items"]
        ]
        column_contents = [
            table["column_contents"] for table in data["schema"]["schema_items"]
        ]
        pk_indicators = [
            table["pk_indicators"] for table in data["schema"]["schema_items"]
        ]
        ...
        if dataset_type == "eval":
            ...
            table_indices = np.argsort(-np.array(table_probs), kind="stable")[
                :num_top_k_tables
            ].tolist()
        elif dataset_type == "train":
            table_indices = [
                table_idx
                for table_idx, table_label in enumerate(data["table_labels"])
                if table_label == 1
            ]
            if len(table_indices) < num_top_k_tables:
                ...
                table_indices += random.sample(
                    unused_table_indices,
                    min(
                        len(unused_table_indices), num_top_k_tables - len(table_indices)
                    ),
                )
        ...
        for table_idx in table_indices:
            if dataset_type == "eval":
                ...
                column_indices = np.argsort(-np.array(column_probs), kind="stable")[
                    :num_top_k_columns
                ].tolist()
            elif dataset_type == "train":
                column_indices = [
                    ...
                    for column_idx, column_label in enumerate(
                        data["column_labels"][table_idx]
                    )
                    if column_label == 1
                ]
                if len(column_indices) < num_top_k_columns:
                    unused_column_indices = [
                        column_idx
                        for column_idx, column_label in enumerate(
                            data["column_labels"][table_idx]
                        )
            ...
            filtered_schema["schema_items"].append(
                {
                    ...
                    "column_names": [
                        column_names[table_idx][column_idx]
                        for column_idx in column_indices
                    ],
                    "column_types": [
                        column_types[table_idx][column_idx]
                        for column_idx in column_indices
                    ],
                    "column_comments": [
                        column_comments[table_idx][column_idx]
                        for column_idx in column_indices
                    ],
                    "column_contents": [
                        column_contents[table_idx][column_idx]
                        for column_idx in column_indices
                    ],
                    "pk_indicators": [
                        pk_indicators[table_idx][column_idx]
                        for column_idx in column_indices
                    ],
                }
            )
            ...
            for column_name in [
                ...
                if tc_name in data["matched_contents"]:
                    filtered_matched_contents[tc_name] = data["matched_contents"][
                        tc_name
                    ]
        ...
        for foreign_key in data["schema"]["foreign_keys"]:
            ...
            if (
                source_table in filtered_table_names
                and target_table in filtered_table_names
            ):
...
class SchemaItemClassifierInference:
    def __init__(self, model_save_path):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            model_save_path, add_prefix_space=True
        )
        # initialize model
        ...
        self.model.load_state_dict(
            torch.load(
                model_save_path + "/dense_classifier.pt",
    ...
    def predict_one(self, sample):
        (
            ...
            table_name_token_indices,
            column_num_in_each_table,
        ) = prepare_inputs_and_labels(sample, self.tokenizer)
        ...
        table_pred_probs = (
            torch.nn.functional.softmax(table_logits, dim=1)[:, 1].cpu().tolist()
        )

        column_logits = model_outputs["batch_column_info_cls_logits"][0]
        column_pred_probs = (
            torch.nn.functional.softmax(column_logits, dim=1)[:, 1].cpu().tolist()
        )
        ...
        for table_id, column_num in enumerate(column_num_in_each_table):
            splitted_column_pred_probs.append(
                column_pred_probs[
                    sum(column_num_in_each_table[:table_id]) : sum(
                        column_num_in_each_table[:table_id]
                    )
                    + column_num
                ]
            )
    ...
    def evaluate_coverage(self, dataset):
        ...
        for data in dataset:
            indices_of_used_tables = [
                idx for idx, label in enumerate(data["table_labels"]) if label == 1
            ]
            pred_results = sic.predict(data)
            # print(pred_results)
            table_probs = [res["table_prob"] for res in pred_results]
            for k in range(max_k):
                indices_of_top_k_tables = np.argsort(
                    -np.array(table_probs), kind="stable"
                )[: k + 1].tolist()
                if lista_contains_listb(
                    indices_of_top_k_tables, indices_of_used_tables
                ):
            ...
            for table_idx in range(len(data["table_labels"])):
                indices_of_used_columns = [
                    idx
                    for idx, label in enumerate(data["column_labels"][table_idx])
                    if label == 1
                ]
                ...
                column_probs = pred_results[table_idx]["column_probs"]
                for k in range(max_k):
                    indices_of_top_k_columns = np.argsort(
                        -np.array(column_probs), kind="stable"
                    )[: k + 1].tolist()
                    if lista_contains_listb(
                        indices_of_top_k_columns, indices_of_used_columns
                    ):
                        column_coverage_results[k] += 1

                total_num_for_column_coverage += 1

                indices_of_top_10_columns = np.argsort(
                    -np.array(column_probs), kind="stable"
                )[:10].tolist()
                if (
                    lista_contains_listb(
                        indices_of_top_10_columns, indices_of_used_columns
                    )
                    == 0
                ):
